    ...
    <div className="max-w-[1124px] mx-auto grid grid-cols-2 items-center h-screen gap-28">
      <main>
        ...
        <div className="mt-10 flex items-center gap-2">
          ...
          <strong className="text-gray-100 text-xl"><span className="text-ignite-500">+{props.userCountResponse}</span> já estão usando</strong>
        </div>
        <form onSubmit={createPool} className="mt-10 flex gap-2" action="">
          <input value={poolTitle} onChange={event => {setPoolTitle(event.target.value)}} className="flex-1 px-6 py-4 rounded bg-gray-800 border border-gray-600 text-sm text-gray-100" type="text" placeholder="Qual nome do seu bolão?" name="" id="" required/>
          <button className="bg-yellow-500 px-6 py-4 rounded font-bold text-gray-900 text-small uppercase hover:bg-yellow-700" type="submit">Criar meu bolão</button>
        </form>
...
export const getServerSideProps = async () => { //APRENDER A USAR getStaticProps NO NEXT.JS PARA NAO FAZER ESSAS CHAMADAS TODO REFRESH.
